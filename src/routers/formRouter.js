...
const keys = JSON.parse(process.env.GOOGLE_KEYS.replace(/•/g, "\n"));
...
router.post("/get-involved", async (req, res) => {
  try {
    const { firstName, lastName, university, email, phoneNumber } = req.body;
...
router.post("/set-up-a-kcsoc", async (req, res) => {
  try {
    ...
    const newRow = [
      [firstName, lastName, email, phoneNumber, moment().format("DD/MM/YYYY")],
    ];
